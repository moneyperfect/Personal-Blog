...
;
;
const GA_ID = ("TURBOPACK compile-time value", "G-XXXXXXXXXX");
...
function trackProductView(productSlug, productTitle) {
    trackEvent('view_item', 'product', `${productSlug}: ${productTitle}`);
}
function trackPurchaseClick(productSlug, productTitle) {
    trackEvent('begin_checkout', 'product', `${productSlug}: ${productTitle}`);
}
function trackWaitlistSubmit(source) {
    trackEvent('generate_lead', 'waitlist', source);
}
function trackResourceCopy(resourceSlug, resourceTitle) {
    trackEvent('copy_content', 'library', `${resourceSlug}: ${resourceTitle}`);
}
function trackResourceDownload(resourceSlug, resourceTitle) {
    trackEvent('download', 'library', `${resourceSlug}: ${resourceTitle}`);
}
}),
"[project]/src/components/PwaRegister.tsx [app-ssr] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
//# sourceMappingURL=%5Broot-of-the-server%5D__20b4155f._.js.map